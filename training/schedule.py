...
class OneCycleSchedule:
    ...
    def __init__(self, t, linear_pct=0.2, eta_max=1.0, eta_min=None,
                 mom_min=0.85, mom_max=0.95, div_factor=100,
                 decay_to_zero=False):
        ...
        self.mom_cosine = CosineAnnealingSchedule(
            eta_min=mom_max,
            eta_max=mom_min,
            t_max=self.t_cosine, t_mult=1)
        self.mom_linear = lambda x: x * (mom_min - mom_max) / self.t_linear + mom_max

        self.momentum = mom_min
        self.iter = 0

    def update(self):
        self.iter += 1
        if self.iter <= self.t_linear:
            self.momentum = self.mom_linear(self.iter)
            return self.linear(self.iter)
        else:
            self.momentum = self.mom_cosine.update()
